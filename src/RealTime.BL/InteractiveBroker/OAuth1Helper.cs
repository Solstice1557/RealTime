...
namespace RealTime.BL.InteractiveBroker
{
    public static class OAuth1Helper
    {
        ...
        public static SortedDictionary<string, string> GetBaseParameters(
            string consumerKey,
            string oauthToken = null,
            bool addCallback = true,
            bool addOAuthVersion = true,
            ...
            string signature)
        {
            ...
            var oauthParameters = parameters.Where(x => x.Key.StartsWith("oauth") || x.Key.Contains("diffie")).ToList();
            if (parameters.ContainsKey("realm"))
            {
                oauthParameters = oauthParameters
                    .Prepend(new KeyValuePair<string, string>("realm", parameters["realm"])).ToList();
            }
            var i = 0;
        ...
        public static string GetNonce(byte length = 16)
        {
